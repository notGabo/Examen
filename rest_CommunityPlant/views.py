...
from .serializers import DescuentoSerializer, BoletaSerializer, SuscriptoresSerializer
...
def lista_suscritos(request):
    """
    lista de los suscritos
    """
    if request.method == 'GET':
        suscritosVar = Usuarios.objects.all()
        serializer = SuscriptoresSerializer(suscritosVar,many=True)
        return Response(serializer.data)
    if request.method == 'POST':
        #data = JSONParser().parse(request)
        serializer = SuscriptoresSerializer(data = request.data)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        else:
